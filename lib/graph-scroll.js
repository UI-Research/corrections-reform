function graphScroll() {
    ...
    function reposition() {
        ...
        if (isBelow != isBelow1) {
            ...
            if (isBelow) {
                //using bootstrap, so graph is in col next to text col
                //use the height of the container to set top attribute in graph-scroll-below
                //NOTE make this be containerHeight - height of the subsection div or something like that
                graph.style("top", (containerHeight - 900) + "px");
            } else {
                graph.style("top", "0px");
            }
        }
    ...
    function resize() {
        containerHeight = Math.round(parseFloat(container.style("height")));
